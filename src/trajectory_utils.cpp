#include <arm_utilities/trajectory_utils.h>
...
void append(trajectory_msgs::JointTrajectory &trajectory1, trajectory_msgs::JointTrajectory const &trajectory2) {
  ...
  if (trajectory1.joint_names.empty()) {
    ...
    RCLCPP_ERROR_STREAM("Tried to append trajectory but the joint names differ");
    RCLCPP_ERROR_STREAM("joint 1 names:");
    for (auto const &name1 : trajectory1.joint_names) {
      RCLCPP_ERROR_STREAM("\t" << name1);
    }
    RCLCPP_ERROR_STREAM("joint 2 names:");
    for (auto const &name2 : trajectory2.joint_names) {
      RCLCPP_ERROR_STREAM("\t" << name2);
    }
