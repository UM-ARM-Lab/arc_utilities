...
#include <arm_utilities/arc_helpers.hpp>
#include <arm_utilities/eigen_typedefs.hpp>
#include <arm_utilities/math_helpers.hpp>
#include <arm_utilities/pretty_print.hpp>
#include <arm_utilities/serialization.hpp>
#include <arm_utilities/simple_robot_model_interface.hpp>
#include <map>
#include <random>
#include <vector>
...
class SimpleJointModel {
 ...
 public:
  ...
  inline uint64_t SerializeSelf(std::vector<uint8_t>& buffer) const {
    ...
    arm_utilities::SerializeFixedSizePOD<double>(limits_.first, buffer);
    arm_utilities::SerializeFixedSizePOD<double>(limits_.second, buffer);
    arm_utilities::SerializeFixedSizePOD<double>(value_, buffer);
    arm_utilities::SerializeFixedSizePOD<uint32_t>((uint32_t)type_, buffer);
  ...
  inline uint64_t DeserializeSelf(const std::vector<uint8_t>& buffer, const uint64_t current) {
    ...
    const std::pair<double, uint64_t> deserialized_limits_first =
        arm_utilities::DeserializeFixedSizePOD<double>(buffer, current_position);
    ...
    const std::pair<uint32_t, uint64_t> deserialized_type =
        arm_utilities::DeserializeFixedSizePOD<uint32_t>(buffer, current_position);
...
class SimpleLinkedConfigSerializer {
 public:
  ...
  static inline uint64_t Serialize(const std::vector<SimpleJointModel>& value, std::vector<uint8_t>& buffer) {
    return arm_utilities::SerializeVector<SimpleJointModel>(value, buffer, SimpleJointModel::Serialize);
  }

  static inline std::pair<std::vector<SimpleJointModel>, uint64_t> Deserialize(const std::vector<uint8_t>& buffer,
                                                                               const uint64_t current) {
    return arm_utilities::DeserializeVector<SimpleJointModel>(buffer, current, SimpleJointModel::Deserialize);
  }
};
