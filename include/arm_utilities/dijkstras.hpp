...
#include <arm_utilities/graph.hpp>
#include <functional>
#include <limits>
#include <queue>
#include <random>
 ...
 public:
  ...
  static uint64_t SerializeDijstrasResult(
    ...
    const auto index_serializer =
        std::bind(arm_utilities::SerializeFixedSizePOD<int64_t>, std::placeholders::_1, std::placeholders::_2);
    SerializeVector(result.second.first, index_serializer);
    // Serialze the distances
    const auto distance_serializer =
        std::bind(arm_utilities::SerializeFixedSizePOD<double>, std::placeholders::_1, std::placeholders::_2);
