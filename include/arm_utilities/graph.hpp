...
class GraphEdge {
 ...
 public:
  ...
  uint64_t serializeSelf(std::vector<uint8_t>& buffer) const {
    ...
    arm_utilities::SerializeFixedSizePOD<int64_t>(from_index_, buffer);
    arm_utilities::SerializeFixedSizePOD<int64_t>(to_index_, buffer);
    arm_utilities::SerializeFixedSizePOD<double>(weight_, buffer);
    arm_utilities::SerializeFixedSizePOD<EDGE_VALIDITY>(edge_validity_, buffer);
...
class GraphNode {
 ...
 public:
  ...
  uint64_t serializeSelf(
    ...
    arm_utilities::SerializeFixedSizePOD<double>(distance_, buffer);
    // Serialize the in edges
    arm_utilities::SerializeVector<GraphEdge>(in_edges_, buffer, GraphEdge::Serialize);
    // Serialize the in edges
    arm_utilities::SerializeVector<GraphEdge>(out_edges_, buffer, GraphEdge::Serialize);
    // Figure out how many bytes were written
    const uint64_t end_buffer_size = buffer.size();
    const uint64_t bytes_written = end_buffer_size - start_buffer_size;
    return bytes_written;
...
class Graph {
 ...
 public:
  ...
  uint64_t serializeSelf(
    ...
    arm_utilities::SerializeVector<GraphNode<NodeValueType, Allocator>>(nodes_, buffer, graph_state_serializer);
    // Figure out how many bytes were written
    const uint64_t end_buffer_size = buffer.size();
    const uint64_t bytes_written = end_buffer_size - start_buffer_size;
    return bytes_written;
