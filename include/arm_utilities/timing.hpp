...
#define PROFILE_REINITIALIZE(prealloc_num_names, prealloc_num_events) \
  ::arm_utilities::Profiler::reset_and_preallocate(prealloc_num_names, prealloc_num_events);

/* Clears the data for a single stored name */
#define PROFILE_RESET(name) ::arm_utilities::Profiler::reset(name)

/* Starts (or restarts) the specified stopwatch */
#define PROFILE_START(name) ::arm_utilities::Profiler::startTimer(name)

/* Record the value of the specified timer. Does not stop or restart the timer */
#define PROFILE_RECORD(name) ::arm_utilities::Profiler::record(name)

/* Record the time of timers and value. Does not stop or restart the timer */
#define PROFILE_RECORD_DOUBLE(name, value) ::arm_utilities::Profiler::recordDouble(name, value)

/* Print a summary of the data for name */
#define PROFILE_PRINT_SUMMARY_FOR_SINGLE(name) ::arm_utilities::Profiler::printSingleSummary(name)

/* Print a condensed summary for each of name in names */
#define PROFILE_PRINT_SUMMARY_FOR_GROUP(names) ::arm_utilities::Profiler::printGroupSummary(names)

#define PROFILE_PRINT_SUMMARY_FOR_ALL() ::arm_utilities::Profiler::printAllSummary()

#define PROFILE_WRITE_SUMMARY_FOR_GROUP(filename, names) ::arm_utilities::Profiler::writeGroupSummary(filename, names)

#define PROFILE_WRITE_SUMMARY_FOR_ALL(filename) ::arm_utilities::Profiler::writeAllSummary(filename)

/* Dumps all data to file */
#define PROFILE_WRITE_ALL(filename) ::arm_utilities::Profiler::writeAll(filename)
...
#define PROFILE_WRITE_ALL_FEWER_THAN(filename, name_count_limit) \
  ::arm_utilities::Profiler::writeAll(filename, name_count_limit)
