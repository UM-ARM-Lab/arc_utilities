...
#include <arm_utilities/serialization_eigen.hpp>
#include <iostream>
#include <sstream>
#include <stdexcept>
#include <string>
...
class VoxelGrid {
 ...
 public:
  ...
  virtual uint64_t SerializeSelf(
    ...
    arm_utilities::SerializeEigen<Eigen::Isometry3d>(inverse_origin_transform_, buffer);
    // Serialize the data
    arm_utilities::SerializeVectorLike<T, BackingStore>(data_, buffer, value_serializer);
  ...
  virtual uint64_t DeserializeSelf(
    ...
    const std::pair<Eigen::Isometry3d, uint64_t> origin_transform_deserialized =
        arm_utilities::DeserializeEigen<Eigen::Isometry3d>(buffer, current_position);
    ...
    const std::pair<BackingStore, uint64_t> data_deserialized =
        arm_utilities::DeserializeVectorLike<T, BackingStore>(buffer, current_position, value_deserializer);
    ...
    const std::pair<int64_t, uint64_t> stride1_deserialized =
        arm_utilities::DeserializeFixedSizePOD<int64_t>(buffer, current_position);
